...
/* 注册一个已有的设备号范围，供驱动使用 */
int register_chrdev_region(dev_t from, unsigned count, const char *name) // 成功返回0，失败返回负的内核错误码
...
int cdev_add(struct cdev *p, dev_t dev, unsigned count) // 成功返回0，失败返回负的内核错误码
...
unsigned long copy_from_user(void *to, const void __user *from, unsigned long n) // 0是成功，>0是部分失败/完全失败，<0是完全失败
```
